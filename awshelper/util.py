...
def call_sts_get_caller_identity(profile_name):
    ...
    try:
        ...
        print(f'AWSHELPER: Unable to make aws call. You may have to run aws configure sso --profile {profile_name}')
        exit(1)
    return
...
def main():
    ...
    if check_cache_expiry(cache_data['Credentials']['Expiration']):
        print(f'AWSHELPER: Credentials cache has expired. Executing Executing aws sts get-caller-identity --profile {profile_name} to attempt to refresh it')
        call_sts_get_caller_identity(profile_name)
